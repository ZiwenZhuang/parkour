...
def namedarraytuple(typename, field_names, return_namedtuple_cls=False,
    ...
    def get_leading_dims(self):
        "Returns the number of dimensions ahead that can be indexed, which means the shape after this dimension will be different beteween fields. This might be longer than the user expected."
        leading_dims = None
        for k, v in self.items():
            if v is None:
                raise AttributeError(f"Field '{k}' is None, you should not get_leading_dims here.")
            if is_namedarraytuple(v):
                leading_dims_v = v.get_leading_dims()
            else:
                leading_dims_v = v.shape
            if leading_dims is None:
                leading_dims = leading_dims_v
            else:
                # check whether the dimension size is the same
                for dim_idx, (l_dim, v_dim) in enumerate(zip(leading_dims, leading_dims_v)):
                    if l_dim != v_dim:
                        break
                leading_dims = leading_dims[:dim_idx]
        return leading_dims
    ...
    class_namespace = {
        ...
        'get': get,
        'items': items,
        'get_leading_dims': get_leading_dims,
    }
